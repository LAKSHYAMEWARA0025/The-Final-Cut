...
const LongFormSection = () => {
  ...
  useEffect(() => {
    // If hovered, do not set the timer
    if (isHovered) return;
  ...
  }, [index, handleNext, isHovered]); // Added isHovered dependency
      ...
      <div className="relative max-w-7xl mx-auto">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            // Added mouse event handlers to pause rotation
            onMouseEnter={() => setIsHovered(true)}
            onMouseLeave={() => setIsHovered(false)}
            // Updated shadow color to Orange (rgba(249,115,22,0.6))
            className="flex flex-col lg:flex-row items-center justify-center gap-10 p-8 rounded-2xl shadow-xl border border-[#2E2E2E] bg-[#1A1A1A] transition-all duration-300 hover:shadow-[0_0_25px_0_rgba(249,115,22,0.6)]"
          >
